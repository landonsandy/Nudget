//
//  MasterViewController.m
//  Nudget
//
//  Created by Landon Sandy on 2/2/13.
//  Copyright (c) 2013 Landon Sandy. All rights reserved.
//

#import "MasterViewController.h"
#import "DetailViewController.h"
#import "Goal.h"

@interface MasterViewController () {
}
@end

@implementation MasterViewController
@synthesize _goals=goals;

- (void)awakeFromNib
{
    if ([[UIDevice currentDevice] userInterfaceIdiom] == UIUserInterfaceIdiomPad) {
        self.clearsSelectionOnViewWillAppear = NO;
        self.contentSizeForViewInPopover = CGSizeMake(320.0, 600.0);
    }
    [super awakeFromNib];
}
- (void)viewDidLoad
{
    goals=[self unarchiveGoals];
    [super viewDidLoad];
	// Do any additional setup after loading the view, typically from a nib.
    self.navigationItem.leftBarButtonItem = self.editButtonItem;

    UIBarButtonItem *addButton = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemAdd target:self action:@selector(insertNewObject:)];
    self.navigationItem.rightBarButtonItem = addButton;
    self.detailViewController = (DetailViewController *)[[self.splitViewController.viewControllers lastObject] topViewController];
}
-(void)viewWillAppear:(BOOL)animated{
    [self refreshControl];
    [self.tableView reloadData];
    [self reloadInputViews];
}
-(void)saveGoals{
    NSArray *_docpath;
    NSString *_dataPath;
    NSString *_userPath;
    _docpath = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
    _dataPath = [[_docpath objectAtIndex:0]stringByAppendingPathComponent:@"UserData"];
    _userPath = [_dataPath stringByAppendingPathComponent:@"user"];
    
    
    if (![[NSFileManager defaultManager] fileExistsAtPath:_dataPath]) {
        [[NSFileManager defaultManager] createDirectoryAtPath:_dataPath
                                  withIntermediateDirectories:NO
                                                   attributes:nil
                                                        error:nil];
    }
    //deep
    
    NSData *goalData =(NSMutableData *)[NSKeyedArchiver archivedDataWithRootObject:goals];
    [goalData writeToFile:_userPath atomically:YES]; //write locally

}

-(NSMutableArray *)unarchiveGoals{
    NSMutableArray *userGoals;
    NSArray *_docpath;
    NSString *_dataPath;
    NSString *_userPath;
    _docpath = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
    _dataPath = [[_docpath objectAtIndex:0]stringByAppendingPathComponent:@"UserData"];
    _userPath = [_dataPath stringByAppendingPathComponent:@"user"];
    
    if (![[NSFileManager defaultManager] fileExistsAtPath:_dataPath]) {
        [[NSFileManager defaultManager] createDirectoryAtPath:_dataPath
                                  withIntermediateDirectories:NO
                                                   attributes:nil
                                                        error:nil];
    }
    NSMutableData *userData ;
    if ([[NSFileManager defaultManager] fileExistsAtPath:_userPath]) {
        userData = [NSMutableData dataWithContentsOfFile:_userPath];
        userGoals=[NSKeyedUnarchiver unarchiveObjectWithData:userData];
    }else{
        userGoals= [[NSMutableArray alloc] init];
    }
    
    return userGoals;
 
}

- (void)didReceiveMemoryWarning
{
    [super didReceiveMemoryWarning];
    // Dispose of any resources that can be recreated.
}

- (void)insertNewObject:(id)sender{
    if (!goals) {
        goals = [[NSMutableArray alloc] init];
    }
    Goal *newGoal = [Goal alloc];
    newGoal.verb=@"do";
    newGoal.object=@"things";
    newGoal.goal =0;
    newGoal.creationDate = [NSDate date];
    [goals insertObject:newGoal atIndex:0];
    NSIndexPath *indexPath = [NSIndexPath indexPathForRow:0 inSection:0];
    [self.tableView insertRowsAtIndexPaths:@[indexPath] withRowAnimation:UITableViewRowAnimationAutomatic];
    [self saveGoals];
}

#pragma mark - Table View

- (NSInteger)numberOfSectionsInTableView:(UITableView *)tableView
{
    return 1;
}

- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
if(!goals)
    goals=[self unarchiveGoals];
    
    return goals.count;
    
    }

- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"Cell" forIndexPath:indexPath];

    Goal *goalForIP = goals[indexPath.row];
    cell.textLabel.text = [NSString stringWithFormat:@"%@ %d %@", goalForIP.verb , goalForIP.goal, goalForIP.object];
    return cell;
}

- (BOOL)tableView:(UITableView *)tableView canEditRowAtIndexPath:(NSIndexPath *)indexPath
{
    // Return NO if you do not want the specified item to be editable.
    return YES;
}

- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        [goals removeObjectAtIndex:indexPath.row];
        [tableView deleteRowsAtIndexPaths:@[indexPath] withRowAnimation:UITableViewRowAnimationFade];
        [self saveGoals];
    } else if (editingStyle == UITableViewCellEditingStyleInsert) {
        // Create a new instance of the appropriate class, insert it into the array, and add a new row to the table view.
    }
}

/*
// Override to support rearranging the table view.
- (void)tableView:(UITableView *)tableView moveRowAtIndexPath:(NSIndexPath *)fromIndexPath toIndexPath:(NSIndexPath *)toIndexPath
{
}
*/

/*
// Override to support conditional rearranging of the table view.
- (BOOL)tableView:(UITableView *)tableView canMoveRowAtIndexPath:(NSIndexPath *)indexPath
{
    // Return NO if you do not want the item to be re-orderable.
    return YES;
}
*/

- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if ([[UIDevice currentDevice] userInterfaceIdiom] == UIUserInterfaceIdiomPad) {
        self.detailViewController.row=indexPath.row;
    }
}

- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    if ([[segue identifier] isEqualToString:@"showDetail"]) {
        NSIndexPath *indexPath = [self.tableView indexPathForSelectedRow];
        [[segue destinationViewController] setRow:indexPath.row];
    }
}

@end
